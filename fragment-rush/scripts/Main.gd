...
# Primeira base jogável 100% procedural para Godot 4.
# Foco: menu bonito, movimento fluido, coleta, obstáculos, rasante perfeito,
# pontuação, moedas, recorde local, loja simples e sensação cósmica.

const SAVE_PATH := "user://fragment_rush_save.json"
const LANES := [-230.0, 0.0, 230.0]
const PLAYER_Y := 980.0
const VIEW_W := 720.0
const VIEW_H := 1280.0
...
var score := 0
var crystals_run := 0
var perfect_grazes := 0
var combo := 0
var best_distance := 0.0
var total_crystals := 0
var selected_skin := "orbe_inicial"
var owned_skins := {"orbe_inicial": true}

var spawn_timer := 0.0
var crystal_spawn_timer := 0.0
var power_spawn_timer := 5.0
var speed := 390.0
var difficulty := 0.0
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventScreenDrag:
		var delta_drag := event.position - touch_start
		if abs(delta_drag.x) > 95.0:
			move_lane(sign(delta_drag.x))
			touch_start = event.position
...
func build_game_nodes() -> void:
	player = Node2D.new()
	player.name = "Player"
	player.position = Vector2(LANES[player_lane], PLAYER_Y)
	add_child(player)
...
func build_ui() -> void:
	...
	for label in [score_label, crystal_label, distance_label, combo_label, status_label]:
	...
	for node in [title_label, best_label, start_button, shop_button]:
...
func update_background(delta: float) -> void:
	for s in stars:
		s["pos"].y += s["speed"] * delta * (1.0 if screen == "game" else 0.28)
		if s["pos"].y > VIEW_H + 10:
			s["pos"] = Vector2(rng.randf_range(0, VIEW_W), -10)
...
func start_game() -> void:
	...
	combo = 0
	spawn_timer = 0.5
	crystal_spawn_timer = 0.1
	power_spawn_timer = 4.5
	speed = 390.0
	difficulty = 0.0
	dash_cooldown = 0.0
	dash_timer = 0.0
...
func show_menu() -> void:
	...
	best_label.text = "Recorde: %dm  •  Cristais: %d\nDesvie do caos. Colete o impossível." % [int(best_distance), total_crystals]
...
func show_shop() -> void:
	...
	hud_layer.visible = false
	shop_info_label.text = "FORMAS / LOJA\n\nCristais disponíveis: %d\n\n✓ Orbe Inicial — Liberado\n\nPróximas formas planejadas:\n• Cometa Violeta — 1.000 cristais\n• Núcleo Solar — 3.000 cristais\n• Sombra Nebular — Fragmento raro\n• Lâmina Cristalina — Evento\n\nEsta base já está estruturada para evoluir a loja com botões de compra, seleção de skins, rastros e poderes." % total_crystals

func update_game(delta: float, real_delta: float) -> void:
	run_time += real_delta
	difficulty += real_delta * 0.018
	speed = min(820.0, 390.0 + distance * 0.03 + difficulty * 42.0)
	distance += speed * delta * 0.045
	score += int(14.0 * delta * (1 + combo * 0.08))
	...
	player.position.x = lerp(player.position.x, target_x, min(1.0, 14.0 * real_delta))
	player.rotation = lerp(player.rotation, (target_x - player.position.x) * 0.003, 8.0 * real_delta)
	player.scale = Vector2.ONE * (1.0 + sin(run_time * 9.0) * 0.025 + (0.11 if dash_timer > 0.0 else 0.0))
	if dash_timer > 0.0:
		spawn_particle(player.position + Vector2(360, PLAYER_Y) * 0.0, Color(0.45, 0.95, 1.0, 0.85), 9, 0.42)
	...
	if spawn_timer <= 0.0:
		...
		spawn_timer = max(0.38, 1.05 - difficulty * 0.025 - distance * 0.0008)
...
func move_lane(dir: int) -> void:
	player_lane = clamp(player_lane + dir, 0, LANES.size() - 1)
	target_x = LANES[player_lane]
	camera_shake = max(camera_shake, 2.0)

func do_dash() -> void:
	if dash_cooldown <= 0.0:
		dash_timer = 0.18
		dash_cooldown = 1.15
		invulnerable_timer = max(invulnerable_timer, 0.2)
		show_status("DASH ASTRAL", Color(0.55, 0.95, 1.0))
		for i in range(12):
			spawn_particle(player.position + Vector2(rng.randf_range(-38, 38), rng.randf_range(-38, 38)), Color(0.3, 0.9, 1.0, 0.8), 7, 0.55)
...
func spawn_obstacle(lane: int, kind: String) -> void:
	entities.append({"type": "obstacle", "kind": kind, "lane": lane, "pos": Vector2(LANES[lane], -90), "radius": 52.0, "scored_graze": false, "rot": rng.randf_range(-1, 1)})

func spawn_crystal_line() -> void:
	var lane := rng.randi_range(0, 2)
	var count := rng.randi_range(3, 6)
	for i in range(count):
		entities.append({"type": "crystal", "kind": "common", "lane": lane, "pos": Vector2(LANES[lane], -80 - i * 72), "radius": 28.0, "value": 1})

func spawn_powerup() -> void:
	var kinds := ["magnet", "shield", "slowmo", "double"]
	entities.append({"type": "power", "kind": kinds[rng.randi_range(0, kinds.size() - 1)], "lane": rng.randi_range(0, 2), "pos": Vector2(LANES[rng.randi_range(0, 2)], -100), "radius": 34.0})
...
func update_entities(delta: float) -> void:
	...
	for i in range(entities.size()):
		...
		if e.type == "obstacle":
			e.rot += delta * 3.2
		var dist_to_player := e.pos.distance_to(player.position)
		if e.type == "crystal":
			if magnet_timer > 0.0 and dist_to_player < 220:
				e.pos = e.pos.lerp(player.position, 8.0 * delta)
			if dist_to_player < 58:
				collect_crystal(e)
				remove_indices.append(i)
		elif e.type == "power":
			if dist_to_player < 62:
				collect_power(e.kind)
				remove_indices.append(i)
		elif e.type == "obstacle":
			if dist_to_player < 66 and invulnerable_timer <= 0.0:
				game_over()
				return
			elif dist_to_player < 118 and not e.scored_graze and abs(e.pos.y - player.position.y) < 70:
				e.scored_graze = true
				perfect_graze()
		if e.pos.y > VIEW_H + 150:
			remove_indices.append(i)
		entities[i] = e
	remove_indices.sort()
...
func collect_crystal(e: Dictionary) -> void:
	var multiplier := 2 if combo >= 8 else 1
	crystals_run += e.value * multiplier
	score += 30 * multiplier
	combo += 1
	spawn_particle(e.pos, Color(0.55, 0.95, 1.0, 0.85), 10, 0.5)
	if combo % 10 == 0:
...
func perfect_graze() -> void:
	...
	score += 90 + combo * 5
	camera_shake = 9.0
	show_status("RASANTE PERFEITO +%d" % (90 + combo * 5), Color(1.0, 0.84, 0.24))
	for i in range(16):
		spawn_particle(player.position + Vector2(rng.randf_range(-42, 42), rng.randf_range(-42, 42)), Color(1.0, 0.78, 0.18, 0.85), 8, 0.45)
...
func spawn_particle(pos: Vector2, color: Color, amount: int, lifetime: float) -> void:
	for i in range(amount):
		particles.append({"pos": pos, "vel": Vector2(rng.randf_range(-160, 160), rng.randf_range(-220, 80)), "color": color, "life": lifetime, "max": lifetime, "size": rng.randf_range(3, 9)})
...
func skin_color(skin: String) -> Color:
	match skin:
		"cometa_violeta": return Color(0.75, 0.35, 1.0, 1.0)
		"nucleo_solar": return Color(1.0, 0.7, 0.18, 1.0)
		"sombra_nebular": return Color(0.24, 0.13, 0.42, 1.0)
		_: return Color(0.55, 0.95, 1.0, 1.0)

func save_game() -> void:
	var data := {"best_distance": best_distance, "total_crystals": total_crystals, "selected_skin": selected_skin, "owned_skins": owned_skins}
...
func load_save() -> void:
	...
	if file:
		...
		if typeof(parsed) == TYPE_DICTIONARY:
			...
			owned_skins = parsed.get("owned_skins", {"orbe_inicial": true})
...
func _draw() -> void:
	...
	for y in range(0, int(VIEW_H), 64):
		var alpha := 0.06 + 0.03 * sin((y + run_time * 40.0) * 0.01)
		draw_rect(Rect2(0, y, VIEW_W, 64), Color(0.08, 0.02, 0.18, alpha))
	for s in stars:
		draw_circle(s.pos, s.size, Color(0.65, 0.9, 1.0, s.alpha))
	# Trilhas/faixas de energia
	for x in LANES:
		draw_line(Vector2(x + VIEW_W / 2.0, 0), Vector2(x + VIEW_W / 2.0, VIEW_H), Color(0.25, 0.75, 1.0, 0.08), 4.0)
	# Converter mundo centralizado para tela
	for e in entities:
		var p: Vector2 = e.pos + Vector2(VIEW_W / 2.0, 0)
		if e.type == "crystal":
			...
			var col := Color(1.0, 0.16, 0.28, 0.88)
			if e.kind == "crystal_spike": col = Color(0.8, 0.18, 1.0, 0.9)
			if e.kind == "barrier": col = Color(1.0, 0.55, 0.12, 0.88)
			if e.kind == "rift": col = Color(0.2, 0.05, 0.35, 0.95)
			draw_obstacle(p, 48.0, col, e.rot)
	for part in particles:
		var alpha := max(0.0, part.life / part.max)
		var c: Color = part.color
		c.a *= alpha
		draw_circle(part.pos + Vector2(VIEW_W / 2.0, 0), part.size * alpha, c)

func draw_crystal(p: Vector2, r: float, color: Color) -> void:
	var pts := PackedVector2Array([Vector2(p.x, p.y - r * 1.4), Vector2(p.x + r, p.y), Vector2(p.x, p.y + r * 1.4), Vector2(p.x - r, p.y)])
...
func draw_obstacle(p: Vector2, r: float, color: Color, rot: float) -> void:
	...
	draw_colored_polygon(pts, color)
	draw_polyline(pts + PackedVector2Array([pts[0]]), Color(1, 1, 1, 0.45), 2.0)
